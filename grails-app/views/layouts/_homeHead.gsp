<%-- THIS JQUERY IS USED FOR SIDEBAR LINKS Active AND Inactive --%>
<% def basePath=(request.getScheme()+':'+'//'+request.getServerName()+':'+request.getServerPort()+request.contextPath) %>
<%-- <base href="${basePath}" /> --%>

<g:set var="path" value="${URLDecoder.decode((request.forwardURI - request.contextPath), 'UTF-8')}"/>
...
$(document).ready(function() {
	 ...
	 var x = $(window).width();
		if(x < 750) {		
			if($(".insideTwitter").length > 0){
				$(".insideTwitter").insertAfter($(".content"));
				$(".insideTwitter").css("clear","both");
				$(".insideTwitter").css("width","90%"); 
				$(".insideTwitter").css("margin","0 auto"); 
			}
		}
